...
const TickerCarousel = () => {
  ...
  const handleNext = () => {
    ...
    setTimeout(() => setIsTransitioning(false), 1200); // Extended for card-switching animation
  };
    ...
    >
      ...
      <div className="relative z-10 h-full p-8 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center relative overflow-hidden">
          {/* Cards container with perspective for 3D effects */}
          <div
            className="relative w-full max-w-5xl h-[480px]"
          ...
          >
            {extendedItems.map((item: MenuItem, index: number) => {
              ...
              if (index === currentIndex) {
                // Current card - enhanced presence with card-flip animation
                ...
                // Next card - positioned like a deck of cards
                ...
                // Previous card - sliding out like a card being discarded
                transform = "translateX(-280px)";
                ...
                // Far previous card - completely hidden
              ...
              return (
                <div
                  key={item.id}
                  className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-1000 ease-in-out cursor-pointer ${
                    isTransitioning
                      ? "transition-all duration-1000 ease-out"
                      : "transition-all duration-700 ease-in-out"
                  }`}
                  style={{
                    transform: `translate(-50%, -50%) ${transform} scale(${scale}) rotateY(${rotateY}deg) translateZ(${translateZ}px)`,
                    opacity,
                    zIndex,
                ...
                >
                  ...
                  >
                    {/* Enhanced gradient border with card-switching glow */}
                    <div
                      className={`absolute -inset-1 bg-gradient-to-r from-red-500 via-amber-500 to-red-500 rounded-2xl opacity-60 group-hover:opacity-90 transition-all duration-500 ${
                        index === currentIndex
                    ...
                    {/* Main card with enhanced card-switching styling */}
                    <div
                      className={`relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl overflow-hidden h-full border border-gray-700 group-hover:border-gray-600 transition-all duration-500 shadow-2xl ${
