...
export const MenuContainer: React.FC<MenuContainerProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (categoryParam && menuCategories.find(cat => cat.id === categoryParam)) {
      ...
      <div className="space-y-8 bg-white rounded-2xl p-8 shadow-lg">
        <div className="text-center">
          ...
          <BodyText className="max-w-2xl mx-auto text-thai-warmGray">
            Discover our most popular authentic Thai dishes, crafted with traditional recipes and premium ingredients.
          </BodyText>
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="text-center mb-20">
        ...
        <BodyText className="text-xl max-w-3xl mx-auto text-thai-warmGray leading-relaxed">
          From timeless classics to modern interpretations, our menu is a celebration of authentic Thai flavors.
        </BodyText>
      </div>
      ...
      <div className="text-center">
        <BodyText className="text-thai-warmGray">
          ...
          {activeCategory !== 'all' && (
            <span> in {menuCategories.find(cat => cat.id === activeCategory)?.name}</span>
          )}
      ...
      {filteredItems.length > 0 ? (
        ...
        <Card variant="transparent" className="text-center p-12 bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl">
          <LanternIcon size={64} color="white" className="mx-auto mb-4 opacity-50" />
          <Heading2 className="text-xl mb-4 text-white">No dishes found</Heading2>
